...
import { addShipment, markShipmentReceived } from '../_actions/projectActions';
...
function AddShipmentModal({ projectId, onClose }) {
  ...
  function handleSubmit(e) {
    ...
    startTransition(async () => {
      const res = await addShipment(projectId, fd);
  ...
  return (
    <Modal title="Add Shipment" subtitle="Log a new shipment on this project" onClose={onClose} maxWidth="max-w-lg">
      <form onSubmit={handleSubmit} className="space-y-4 px-6 py-5">
        ...
        <Field label="Vendor" required>
          <input name="vendor" type="text" required className={INPUT_CLS} />
        </Field>

        <Field label="Category" required>
          <select name="category" defaultValue={CATEGORIES[0]} className={INPUT_CLS}>
            {CATEGORIES.map((c) => (
        ...
        <Field label="Description" required>
          <input name="description" type="text" required className={INPUT_CLS} />
        </Field>
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <Field label="Cartons">
            <input name="cartons" type="number" min="0" defaultValue="0" className={INPUT_CLS} />
          </Field>
          <Field label="ETA">
            <input name="eta" type="date" className={INPUT_CLS} />
          </Field>
        </div>
...
export default function ShipmentsTab({ shipments, projectId }) {
  ...
  return (
    <div className="space-y-3">
      ...
      {showAdd && (
        <AddShipmentModal projectId={projectId} onClose={() => setShowAdd(false)} />
      )}
    </div>
  );
}
